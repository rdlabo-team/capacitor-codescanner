...
public class CodeScannerBottomSheetFragment extends BottomSheetDialogFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        MaterialButton scanButton = view.findViewById(R.id.scan_button);
        FrameLayout cameraPreviewContainer = view.findViewById(R.id.camera_preview);
        scanButton.setOnClickListener(v -> {
            if (!isCameraStarted) {
                // カメラ権限をチェック
                if (ContextCompat.checkSelfPermission(requireContext(), Manifest.permission.CAMERA) 
                        == PackageManager.PERMISSION_GRANTED) {
                    startCamera(cameraPreviewContainer);
    ...
    private void startCamera(FrameLayout container) {
